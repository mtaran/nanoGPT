...
wandb_log = True
wandb_project = 'owt'
wandb_run_name = 'run' + str(time.strftime("%m-%dT%H:%M:%S"))
# data
dataset = 'openwebtext'
...
def main():
    ...
    while True:
        ...
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
        if iter_num < 5:
            log(f"step {iter_num}, evaluating loss to {lr:.2e}")
        ...
        if iter_num % eval_interval == 0 and master_process:
            ...
            if wandb_log:
                wandb.log({
                    ...
                    "val/loss": losses['val'],
                    "lr": lr,
                    "mfu": max(0, running_mfu*100), # convert to percentage
                })
        ...
        for micro_step in range(gradient_accumulation_steps):
            if iter_num < 5:
                log(f"step {iter_num}, micro-step {micro_step}")
            if ddp:
            ...
            if iter_num < 5:
                log(f"step {iter_num}, micro-step {micro_step}, loss {loss.item()}")
        ...
        if iter_num % log_interval == 0 and master_process:
            ...
            if iter_num < 5:
                log(f"step {iter_num}, now logging")
